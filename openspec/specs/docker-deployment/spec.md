# Docker 容器化部署規格 (Docker Deployment Specification)
...
系統 SHALL 提供多階段 Dockerfile，於第一階段（Builder）採用 `maven:3.9-eclipse-temurin-17` 自動編譯打包產出 `target/jtrac.war`，於第二階段（Runtime）採用官方 `jetty:12-jre17-eclipse-temurin` 映像檔，並透過 `--add-modules=ee8-deploy,ee8-webapp` 啟用 EE8 執行環境以支援 Servlet 4.0 (`javax.servlet`) 規格。
