...
/**
 * This class is responsible the panel showing the item read-only view.
 */
public class ItemViewPanel extends BasePanel {
    ...
    private void addComponents(final Item item) {
        Map<String, String> configMap = getJtrac().loadAllConfig();
		String baseUrl = configMap.get("jtrac.url.base");
		...
		if (renderMarkdown())
			text = ItemUtils.renderMarkdown(text);
		else
			text = ItemUtils.autolinkUrls(text);
		text = ItemUtils.autolinkTickets(baseUrl, text, getPrincipal().getSpaces());
        add(new Label("detail", text).setEscapeModelStrings(false));
        ...
        if (item.getHistory() != null) {
            ...
            add(new ListView("history", history) {
                ...
                protected void populateItem(ListItem listItem) {
					...
					String text = h.getComment();
					if (renderMarkdown())
						text = ItemUtils.renderMarkdown(text);
					else
						text = ItemUtils.autolinkUrls(text);
					text = ItemUtils.autolinkTickets(baseUrl, text, getPrincipal().getSpaces());
                    comment.add(new Label("comment", text).setEscapeModelStrings(false));
