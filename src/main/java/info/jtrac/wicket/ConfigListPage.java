 ...
 * It also declares which edit forms will be called to modify the configuration parameters.
 */

public class ConfigListPage extends BasePage {

	private static final SimpleAttributeModifier SAM_ALT = new SimpleAttributeModifier("class", "alt");
	private static final SimpleAttributeModifier SAM_SEL = new SimpleAttributeModifier("class", "selected");
        ...
        add(new ListView("configs", params) {
            protected void populateItem (ListItem listItem) {
                ...
                if (param.equals(selectedParam)) {
                    listItem.add(SAM_SEL);
                } else if (listItem.getIndex() % 2 == 1) {
                    listItem.add(SAM_ALT);
                }
                ...
                if (param.toLowerCase().indexOf("password") != -1) {
					...
					// obscure the password, but give an indication of its length
					if (StringUtils.hasText(value))
						listItem.add(new Label("value", new String(new char[value.length()]).replaceAll("\0", "*")));
					else
