...
public class ItemViewPage extends BasePage {              
    ...
    private void addComponents(final Item item) {  
        ...
        String canEdit = configMap.get("jtrac.edit.item");        
        String canCommentClosed = configMap.get("jtrac.comment.closed");        
        ...
        if (user.isGuestForSpace(item.getSpace())
				|| ((item.getStatus() == State.CLOSED) && "false".equals(canCommentClosed))
				|| isRelate) { 
