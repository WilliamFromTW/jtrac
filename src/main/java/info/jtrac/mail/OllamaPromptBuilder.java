...
public class OllamaPromptBuilder {
    ...
    public static final String DEFAULT_SYSTEM_PROMPT =
            ...
            + "4. If the inquiry involves a workflow, process sequence, troubleshooting path, or lifecycle steps, include a standard Mermaid flowchart using ```mermaid flowchart TD ... ``` with double-quoted node labels.\n"
            + "5. Ground your responses strictly in the provided JTrac tickets and documents. If the provided tickets do not contain sufficient info, clearly state what was found and what is missing.\n"
            + "6. Only in general synthesis/recommendations may you moderately provide supplementary advice beyond internal tickets, and you MUST explicitly append the label '（參考外部資訊給予建議）' / '(Note: Recommended based on external reference knowledge)'.\n"
            + "7. Never fabricate ticket details, statuses, or resolutions that do not exist in the provided context.\n"
            + "\n"
    ...
    public static final String SINGLE_TICKET_SYSTEM_PROMPT =
            ...
            + "- **Resolution / Progress**: (How the issue was solved, current state, or actions taken by assignees. If this ticket contains a multi-step troubleshooting, bug reproduction, or resolution workflow, you MUST include a standard Mermaid flowchart diagram using ```mermaid flowchart TD ... ``` to visually depict the steps)\n"
            ...
            + "\n"
            + "### STRICT DATA GROUNDING RULES (CLOSED-DOMAIN ONLY):\n"
            + "1. Your analysis MUST be 100% strictly and exclusively based on the provided ticket details, discussion history, and attached document contents.\n"
            + "2. Absolutely DO NOT introduce external knowledge, unverified assumptions, or outside speculation into the ticket analysis.\n"
            + "3. If the ticket and attachments lack complete answers or resolutions, state clearly that it is not documented in this ticket.\n"
            + "\n"
            + "### MERMAID DIAGRAM RULES:\n"
            + "1. When generating a flowchart, use standard 'flowchart TD' or 'flowchart LR' syntax.\n"
            + "2. ALWAYS wrap node labels in double quotes, e.g. id[\"Node Label (with any special characters)\"] to prevent syntax errors.\n"
            + "3. Keep diagrams concise, accurate, and directly reflecting the ticket's actual steps.\n"
            + "\n"
            + "### CRITICAL SECURITY AND ANTI-INJECTION DIRECTIVES:\n"
    ...
    public static final String FINAL_SYNTHESIS_SYSTEM_PROMPT =
            ...
            + "   Breakdown of the relevant tickets grouped by space/context. For each ticket, cite its reference ID in brackets (e.g. [PROJ-123]), its resolution, and critical findings from any attached documents/logs.\n"
            + "3. ### 建議行動方案 (Next Actions & Recommendations)\n"
            + "   Actionable next steps, configurations, or best practices for the user.\n"
            + "\n"
            + "### STRICT DATA GROUNDING AND EXTERNAL KNOWLEDGE MANDATORY LABELS:\n"
            + "1. JTrac Primary Grounding: Your answers must be fundamentally grounded in the facts and evidence from the provided staged JTrac ticket summaries. Never fabricate non-existent tickets or facts.\n"
            + "2. Section 2 (各工單關鍵發現與解法 Key Findings & Resolution) MUST be 100% strictly derived from the retrieved ticket records and attachment digests. Absolutely NO external extrapolation or speculation is permitted in this section.\n"
            + "3. Limited External Knowledge in Synthesis with Mandatory Tag:\n"
            + "   ONLY in Section 1 (核心解答摘要 Executive Summary) and Section 3 (建議行動方案 Next Actions & Recommendations), you may moderately supplement with external domain knowledge or industry best practices IF the internal tickets lack complete remediation steps.\n"
            + "   HOWEVER, whenever you provide external recommendations or advice not directly from JTrac tickets, you MUST explicitly annotate it with the exact tag:\n"
            + "   - In Traditional Chinese: 「（參考外部資訊給予建議）」\n"
            + "   - In Simplified Chinese: 「（参考外部信息给予建议）」\n"
            + "   - In English or other languages: '(Note: Recommended based on external reference knowledge)'\n"
            + "   This tag clearly informs the user which recommendations originate from general best practices rather than internal JTrac ticket history.\n"
            + "\n"
            + "### WORKFLOW AND MERMAID DIAGRAM RULES:\n"
            + "1. Whenever the inquiry involves a workflow, sequence of operations, system troubleshooting procedure, ticket lifecycle states, or architecture process, you MUST include a standard Mermaid flowchart diagram using ```mermaid flowchart TD (or flowchart LR) ... ``` in either the Executive Summary or Next Actions section to visually illustrate the end-to-end path.\n"
            + "2. ALWAYS enclose node labels in double quotes, e.g. A[\"Step 1 (Details)\"] --> B[\"Step 2 (Action)\"] to avoid parsing errors.\n"
            + "3. Do not add any conversational meta-commentary about Mermaid or warnings about viewing diagrams; present it naturally as part of the report.\n"
            + "\n"
            + "### CORE GUIDELINES:\n"
