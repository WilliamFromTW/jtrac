...
public class HibernateJtracDao extends HibernateDaoSupport implements JtracDao {
    ...
    public List<StoredSearch> findAllStoredSearch() {
        List<StoredSearch> searches = getHibernateTemplate().loadAll(StoredSearch.class);
    ...
    /**
     * note that this is automatically configured to run on startup as a spring bean "init-method"
     */
    public void createSchema() {
        try {
            getHibernateTemplate().find("from Item item where item.id = 1");
            logger.info("database schema exists, normal startup with updating the schema, if necessary");
		...
		// default values for new attributes; haven't found a way to set default values when Hibernate creates these
		getHibernateTemplate().bulkUpdate("update Space space set space.isActive = true where space.isActive is null");
		getHibernateTemplate().bulkUpdate("update StoredSearch search set search.newWindow = true where search.newWindow is null");
		getHibernateTemplate().bulkUpdate("update User user set user.prettyDates = true where user.prettyDates is null");
