...
7. [Docker Operations & Container Deployment](#7-docker-operations--container-deployment)
...
## 7. Docker Operations & Container Deployment

When running JTrac in a Docker container environment, all database files, attachments, and configurations are persisted in the container's `/jtrac-data` directory.

For comprehensive documentation on Docker image building, Docker Hub deployment, container startup options, cross-platform scripts, and maintenance, please refer directly to the dedicated Docker guide:
