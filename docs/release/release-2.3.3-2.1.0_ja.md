...
4. **オフライン対応 HTML レポート添付 (`JTrac-AI-Report-[yyyyMMdd-HHmm].html`)**：
   - **メール本文の極小化**：チケット一覧とリンクのみを掲載し、メーラーのレイアウト崩れを完全に防止。
   - **完全自立型 HTML 添付**：メモリ上で `ByteArrayResource` として即座に生成・配信（3ミリ秒未満、ディスク I/O ゼロ）。
   - **モダンデザイン**：テーブル罫線の明示（`border-collapse: collapse`）、ネイティブ `<details>` アコーディオン、ダークモード自動連動、印刷時全展開スタイルを完備。
