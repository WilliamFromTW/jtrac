...
- **嚴格的版本控制 (Strict Version Control)**：在完成任何重大更新或功能後，永遠必須主動執行 `git add .` 與 `git commit`。所有 Git Commit 訊息一律強制使用英文撰寫，遵循 Conventional Commits 規範（例如 `feat: ...`、`fix: ...`、`docs: ...`、`refactor: ...`、`chore: ...`），嚴禁使用任何非英文提交訊息。
